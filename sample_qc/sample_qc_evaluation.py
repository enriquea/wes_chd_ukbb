...
Compute stratified sample QC metrics on common/rare filtered variants to evaluate the sample QC process.
...
Variant filters:
 ...
 - removed SegDups
 - coding variants (CDS)
 - within capture intervals (intersect of capture intervals used in this project)

Export result as separated HailTables. Optionally, export results as GBZ-compresed TSV files.
...
def compute_sample_qc(mt: hl.MatrixTable) -> hl.Table:
    ...
    mt = filter_to_cds_regions(mt)

    # filter to capture intervals
    mt = filter_capture_intervals(mt)

    # compute cohort-specific af
    mt = (mt
          .annotate_rows(af=hl.agg.mean(mt.GT.n_alt_alleles()) / 2)
          )

    sample_qc_ht = compute_stratified_sample_qc(
        mt,
        strata={
            "rare": af_filter_expr(mt, af_field='af'),
            "common": ~af_filter_expr(mt, af_field='af'),
        }, tmp_ht_prefix=None,
        gt_expr=None
    )
